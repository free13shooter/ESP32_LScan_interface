...
WiFiClient WiFiServer::available(){
  ...
  struct sockaddr_in _client;
  int cs = sizeof(struct sockaddr_in);
  //ESP_LOGI("server", "attempt accept %d...",sockfd);
#ifdef ESP_IDF_VERSION_MAJOR
    client_sock = lwip_accept(sockfd, (struct sockaddr *)&_client, (socklen_t*)&cs);
#else
    client_sock = lwip_accept_r(sockfd, (struct sockaddr *)&_client, (socklen_t*)&cs);
  ...
  if(client_sock >= 0){
    //ESP_LOGI("WiFiServer", "socket %d accepted",client_sock);
    int val = 1;
    if(setsockopt(client_sock, SOL_SOCKET, SO_KEEPALIVE, (char*)&val, sizeof(int)) == ESP_OK) {
      val = _noDelay;
      if(setsockopt(client_sock, IPPROTO_TCP, TCP_NODELAY, (char*)&val, sizeof(int)) == ESP_OK)
          return WiFiClient(client_sock);
        else ESP_LOGE("WiFiServer", "TCP_NODELAY fail !!!");
    }
    else{
      ESP_LOGE("WiFiServer", "SO_KEEPALIVE fail !!!");
    }
  }
  return WiFiClient();
}
